...
public class AliasObject <T> {
	...
	public AliasObject(T t) {
		type = t;
		attributes = new HashMap<String, ArrayList<AliasObject <?>>>(type.getClass().getDeclaredFields().length);
		setVisited(false);
	}
	...
	public void addAttribute (String attributeName)  {
		...
		try {
			attributes.get(attributeName).add(new AliasObject (type.getClass().getDeclaredField(attributeName).getType()));
	...
	public static void main (String[] arg) throws NoSuchFieldException, SecurityException {
		AliasObject<sourceClass> t = new AliasObject<sourceClass>(new sourceClass());
